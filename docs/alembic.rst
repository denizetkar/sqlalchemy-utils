...
.. warning::
   ...
   ``with_data`` **diverges between runtime and autogenerate.** At runtime,
   ``op.create_materialized_view(..., with_data=True)`` populates the MV
   immediately. Autogenerate, however, always emits ``with_data=False``
   (unpopulated) to avoid expensive data population during migrations.
