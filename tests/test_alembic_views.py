...
class TestRendererCreateView:
    ...
    def test_renders_cascade_on_drop_false(self):
        """``cascade_on_drop=False`` must be rendered so the autogenerated
        downgrade (reverse → DropViewOp) honors the non-cascading drop.

        Without rendering ``cascade_on_drop=False``, the rendered
        ``op.create_view(...)`` defaults to ``cascade_on_drop=True``, so the
        autogenerated downgrade incorrectly uses ``DROP VIEW ... CASCADE``.
        """
        op = CreateViewOp("v", "SELECT 1", cascade_on_drop=False)
        rendered = render_create_view(_make_autogen_context(), op)
        assert "cascade_on_drop=False" in rendered, (
            f"Expected cascade_on_drop=False in rendered create_view; "
            f"got: {rendered!r}"
        )

    def test_omits_cascade_on_drop_when_default_true(self):
        """``cascade_on_drop=True`` (the default) is NOT rendered."""
        op = CreateViewOp("v", "SELECT 1")
        rendered = render_create_view(_make_autogen_context(), op)
...
class TestRendererCreateMaterializedView:
    ...
    def test_renders_cascade_on_drop_false(self):
        """``cascade_on_drop=False`` must be rendered so the autogenerated
        downgrade (reverse → DropMaterializedViewOp) honors the non-cascading
        drop. Without rendering it, the downgrade defaults to CASCADE."""
        op = CreateMaterializedViewOp("mv", "SELECT 1", cascade_on_drop=False)
        rendered = render_create_materialized_view(_make_autogen_context(), op)
        assert "cascade_on_drop=False" in rendered, (
            f"Expected cascade_on_drop=False in rendered "
            f"create_materialized_view; got: {rendered!r}"
        )
