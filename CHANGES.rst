...
- Add ``ViewReadonlyError`` exception (subclasses ``sa.exc.InvalidRequestError``); raised
  via a ``before_flush`` listener on ``ViewMixin`` — writes to view-backed ORM instances
  now raise instead of being undefined.
...
- ``ViewMixin`` uses ``__view_cascade__`` for cascade control.
...
- ``CreateViewOp`` and ``CreateMaterializedViewOp`` accept ``cascade_on_drop=``
  (defaults ``True``); the autogenerated downgrade honors it to control
  ``CASCADE`` on ``DROP VIEW``/``DROP MATERIALIZED VIEW``.
- ``create_materialized_view()`` newly accepts ``cascade_on_drop=`` (defaults
  ``True``); when ``True``, the view is dropped with ``CASCADE``.
  ``create_view()`` already had ``cascade_on_drop=`` (defaults ``True``).
- Changed ``RefreshMaterializedView.inherit_cache`` to ``cache_ok = True``
  (SQLAlchemy 2.0 caching API).
- ``with_data`` diverges between runtime and autogenerate: the runtime
  ...
  for which PostgreSQL defaults to ``WITH DATA`` (immediate population).
  The Alembic ``op.create_materialized_view()`` /
  ``op.replace_materialized_view()`` operations default to ``WITH NO DATA``
  (``with_data=False``) to avoid expensive data population during migrations.
  After applying an autogenerated ``create_materialized_view`` or
...
- **Breaking:** ``schema`` is keyword-only on ``create_view``, ``create_materialized_view``,
  ``create_table_from_selectable``, ``CreateView``, ``DropView``, ``RefreshMaterializedView``,
  and ``refresh_materialized_view`` for consistency with the Alembic op API. Likewise,
  ``concurrently`` is keyword-only on ``refresh_materialized_view`` and
  ``RefreshMaterializedView`` (was positional).
- ``create_view`` accepts ``replace=`` (defaults ``False``); this parameter
  was already present on ``create_view()`` but is now propagated to the
  autogenerated ``ReplaceViewOp`` during autogenerate. When ``True``, emits
...
- ``DropViewOp`` / ``DropMaterializedViewOp`` accept ``definition=`` (and
  ``with_data=`` for MV) so the autogenerated downgrade can re-create the
  dropped view; ``reverse()`` raises ``NotImplementedError`` when absent.
