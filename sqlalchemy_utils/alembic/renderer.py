...
def render_drop_view(autogen_context: AutogenContext, op: DropViewOp) -> str:
    schema_part = f", schema={op.schema!r}" if op.schema else ""
    mat_part = ", materialized=True" if op.materialized else ""
    cascade_part = ", cascade=True" if op.cascade else ""
    return f"op.drop_view({op.name!r}{schema_part}{mat_part}{cascade_part})"
...
def render_create_materialized_view(
    ...
    schema_part = f", schema={op.schema!r}" if op.schema else ""
    return f"op.create_materialized_view({op.name!r}, {op.definition!r}{schema_part}, with_data=False)"
...
def render_replace_materialized_view(
    ...
    schema_part = f", schema={op.schema!r}" if op.schema else ""
    return f"op.replace_materialized_view({op.name!r}, {op.definition!r}{schema_part}, with_data=False)"
