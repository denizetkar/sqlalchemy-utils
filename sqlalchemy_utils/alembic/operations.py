...
class CreateViewOp(MigrateOperation):
    """Operation that emits ``CREATE VIEW``.
    ...
    :param cascade_on_drop: When ``True`` (default), the autogenerated
        ...
        are removed as well.
    :raises TypeError: if *name* or *definition* is not a non-empty string.
    """
    ...
    def to_diff_tuple(self) -> tuple[object, ...]:
        return ("create_view", self.name, self.definition, self.schema, self.cascade_on_drop)
...
class DropViewOp(MigrateOperation):
    ...
    :param definition: Optional stored definition of the view. When
        ...
        ``NotImplementedError``.
    :raises TypeError: if *name* is not a non-empty string, or if
        *definition* is provided but is not a non-empty string.
    """
    ...
    def to_diff_tuple(self) -> tuple[object, ...]:
        return ("drop_view", self.name, self.schema, self.cascade, self.definition)


@Operations.register_operation("replace_view")
class ReplaceViewOp(MigrateOperation):
    """Operation that emits ``DROP VIEW`` + ``CREATE VIEW``.
    ...
    :param schema: Optional schema name; when given, the view name is
        schema-qualified in the emitted DDL.
    :param cascade: When ``True`` (default), appends ``CASCADE`` to the
        ``DROP VIEW`` so dependent views do not block the drop. When
        ``False``, emits a plain ``DROP VIEW`` (the drop will fail if
    ...
    def replace_view(
        ...
        *,
        ...
        """
        op = ReplaceViewOp(
            name, definition, schema=schema, cascade=cascade,
    ...
    def to_diff_tuple(self) -> tuple[object, ...]:
        return (
            "replace_view", self.name, self.definition, self.schema,
            self.cascade, self.old_definition,
        )
...
class CreateMaterializedViewOp(MigrateOperation):
    ...
    def to_diff_tuple(self) -> tuple[object, ...]:
        return (
            "create_materialized_view", self.name, self.definition,
            self.schema, self.with_data, self.cascade_on_drop,
        )
...
class DropMaterializedViewOp(MigrateOperation):
    ...
    def to_diff_tuple(self) -> tuple[object, ...]:
        return (
            "drop_materialized_view", self.name, self.schema,
            self.cascade, self.definition, self.with_data,
        )
...
class ReplaceMaterializedViewOp(MigrateOperation):
    ...
    def to_diff_tuple(self) -> tuple[object, ...]:
        return (
            "replace_materialized_view", self.name, self.definition,
            self.schema, self.with_data, self.cascade, self.old_definition,
        )
...
class RefreshMaterializedViewOp(MigrateOperation):
    ...
    .. note:: Materialized views are PostgreSQL-specific; other dialects
       will raise at execute time.
    :raises TypeError: if *name* is not a non-empty string.
    """
    ...
    def to_diff_tuple(self) -> tuple[object, ...]:
        return ("refresh_materialized_view", self.name, self.schema, self.concurrently)
...
def _drop_impl(
    operations: Operations, op: DropViewOp | DropMaterializedViewOp, view_type: str
) -> None:
...
def _replace_impl(
    operations: Operations,
    op: ReplaceViewOp | ReplaceMaterializedViewOp,
    view_type: str,
    data_clause: str = "",
) -> None:
    """Drop then re-create a view (regular or materialized).

    Builds ``DROP {view_type}VIEW IF EXISTS ... [CASCADE]`` followed by
    ``CREATE {view_type}VIEW ... AS ...{data_clause}`` and executes both
    statements on the migration connection. ``view_type`` is ``""`` for a
    regular view or ``"MATERIALIZED "`` for a materialized view;
    ``data_clause`` is the trailing `` WITH DATA``/`` WITH NO DATA``
    fragment for materialized views and ``""`` for regular views.
    """
    dialect = operations.get_bind().dialect
    qualified = _quote_qualified_name(dialect, op.name, op.schema)
    cascade_clause = " CASCADE" if op.cascade else ""
    drop_sql = f"DROP {view_type}VIEW IF EXISTS {qualified}{cascade_clause}"
    create_sql = (
        f"CREATE {view_type}VIEW {qualified} AS {op.definition}{data_clause}"
    )
...
def _replace_view_impl(operations: Operations, op: ReplaceViewOp) -> None:
    """Drop then re-create a regular view via the migration connection.

    Uses ``DROP VIEW IF EXISTS ...`` + ``CREATE VIEW`` instead of
    ``CREATE OR REPLACE VIEW``. PG refuses ``CREATE OR REPLACE VIEW``
    when the new view's column structure differs from the existing view
    (e.g. removing or reordering columns), which would make generated
    migrations un-runnable for column-structure changes. DROP+CREATE has
    no such restriction. ``CASCADE`` is appended when the op's cascade
    flag is True (the default) so dependent views do not block the DROP.
    This mirrors ``_replace_materialized_view_impl`` and the
    comparator's ``_build_create_sql``.
    """
    _replace_impl(operations, op, "")
...
def _replace_materialized_view_impl(
    ...
    data_clause = " WITH DATA" if op.with_data else " WITH NO DATA"
    _replace_impl(operations, op, "MATERIALIZED ", data_clause)
