...
class CreateViewOp(MigrateOperation):
    ...
    :param replace: Deprecated. When ``True``, emits ``DROP VIEW`` +
        ``CREATE VIEW`` (same as :class:`ReplaceViewOp`); prefer
        ``op.replace_view()`` / :class:`ReplaceViewOp` instead, whose
        ``reverse()`` restores the prior definition.
...
def _create_view_impl(operations: Operations, op: CreateViewOp) -> None:
    """Execute ``CREATE VIEW`` (or ``DROP``+``CREATE`` when ``replace=True``) via the migration connection."""
    if op.replace:
...
def _replace_view_impl(operations: Operations, op: CreateViewOp | ReplaceViewOp) -> None:
    ...
    ``ReplaceViewOp`` exposes ``cascade``; the deprecated
    ``CreateViewOp(replace=True)`` path exposes the same semantics via
    ``cascade_on_drop``.
    """
    dialect = operations.get_bind().dialect
    qualified = _quote_qualified_name(dialect, op.name, op.schema)
    if isinstance(op, ReplaceViewOp):
        ...
        cascade = op.cascade_on_drop
    cascade_clause = " CASCADE" if cascade else ""
    drop_sql = f"DROP VIEW IF EXISTS {qualified}{cascade_clause}"
    create_sql = f"CREATE VIEW {qualified} AS {op.definition}"
