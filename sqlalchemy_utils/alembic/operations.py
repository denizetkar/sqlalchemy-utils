...
class DropMaterializedViewOp(MigrateOperation):
    ...
    .. note::
       ``with_data`` only affects the autogenerated downgrade (reverse);
       the ``DROP`` itself ignores it. ``with_data`` is carried solely so
       that :meth:`reverse` can restore the materialized view with the
       original ``WITH DATA`` / ``WITH NO DATA`` clause.
    """
