...
class CreateViewOp(MigrateOperation):
    ...
    def create_view(
        ...
        *,
        ...
        :param operations: The Alembic :class:`~alembic.operations.Operations`
            instance passed to the migration (``op``).
        :param name: Name of the view to create.
        :param definition: SQL definition (the ``SELECT`` body), as a string.
        :param schema: Optional schema name. Keyword-only.
        :param replace: If ``True``, emit ``CREATE OR REPLACE VIEW``.
            Deprecated — prefer :meth:`replace_view` so the autogenerated
            downgrade can restore the prior definition. Keyword-only.
        :param cascade_on_drop: If ``True`` (default), the autogenerated
            downgrade (reverse) drops the view with ``CASCADE``. Set to
            ``False`` to emit a non-cascading drop. Keyword-only.
...
class DropViewOp(MigrateOperation):
    ...
    def drop_view(
        ...
        *,
        ...
        :param schema: Optional schema name. Keyword-only.
        :param cascade: If ``True`` (default), emit ``DROP VIEW ... CASCADE``.
            Keyword-only.
        :param definition: Optional prior SQL definition stored so the
            autogenerated downgrade can re-create the view. Keyword-only.
...
class ReplaceViewOp(MigrateOperation):
    ...
    def replace_view(
        ...
        *,
        ...
        :param definition: New SQL definition (the ``SELECT`` body), as a string.
        :param schema: Optional schema name. Keyword-only.
        :param old_definition: Prior SQL definition stored so the
            autogenerated downgrade can restore it. Keyword-only.
...
class CreateMaterializedViewOp(MigrateOperation):
    ...
    def create_materialized_view(
        ...
        *,
        ...
        :param definition: SQL definition (the ``SELECT`` body), as a string.
        :param schema: Optional schema name. Keyword-only.
        :param with_data: If ``True``, emit ``WITH DATA`` (populate the
            materialized view at creation). Defaults to ``False``
            (``WITH NO DATA``); autogenerate also emits ``WITH NO DATA``.
            Keyword-only.
        :param cascade_on_drop: If ``True`` (default), the autogenerated
            downgrade (reverse) drops the materialized view with
            ``CASCADE``. Set to ``False`` to emit a non-cascading drop.
            Keyword-only.
...
class DropMaterializedViewOp(MigrateOperation):
    ...
    def drop_materialized_view(
        ...
        *,
        ...
        :param schema: Optional schema name. Keyword-only.
        :param cascade: If ``True`` (default), emit
            ``DROP MATERIALIZED VIEW ... CASCADE``. Keyword-only.
        :param definition: Optional prior SQL definition stored so the
            autogenerated downgrade can re-create the materialized view.
            Keyword-only.
        :param with_data: Only affects the autogenerated downgrade (reverse);
            the ``DROP`` itself ignores it. Forwarded so :meth:`reverse`
            can re-create the materialized view with the original
            ``WITH DATA`` / ``WITH NO DATA`` clause. Keyword-only.
...
class ReplaceMaterializedViewOp(MigrateOperation):
    ...
    def replace_materialized_view(
        ...
        *,
        ...
        :param definition: New SQL definition (the ``SELECT`` body), as a string.
        :param schema: Optional schema name. Keyword-only.
        :param with_data: If ``True``, re-create with ``WITH DATA``.
            Defaults to ``False`` (``WITH NO DATA``). Keyword-only.
        :param cascade: If ``True`` (default), the internal
            ``DROP MATERIALIZED VIEW`` uses ``CASCADE``. Keyword-only.
        :param old_definition: Prior SQL definition stored so the
            autogenerated downgrade can restore it. Keyword-only.
...
class RefreshMaterializedViewOp(MigrateOperation):
    ...
    def refresh_materialized_view(
        ...
        *,
        ...
        :param schema: Optional schema name. Keyword-only.
        :param concurrently: If ``True``, emit
            ``REFRESH MATERIALIZED VIEW CONCURRENTLY``. The materialized
            view must have at least one ``UNIQUE`` index. Keyword-only.
