...
def _replace_view_impl(operations: Operations, op: CreateViewOp | ReplaceViewOp) -> None:
    ...
    Uses ``DROP VIEW IF EXISTS ... CASCADE`` + ``CREATE VIEW`` instead of
    ...
    no such restriction. CASCADE is needed so dependent views do not
    block the DROP. This mirrors ``_replace_materialized_view_impl`` and
    the comparator's ``_build_create_sql``.
    """
    dialect = operations.get_bind().dialect
    qualified = _quote_qualified_name(dialect, op.name, op.schema)
